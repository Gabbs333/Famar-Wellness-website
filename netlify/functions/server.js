...
app.post('/api/book', async (req, res) => {
  ...
  // Save to Google Calendar (if configured)
  if (!auth) {
    console.log('Missing Google Credentials - Simulating Booking Success');
    console.log('Booking Data:', req.body);
    await new Promise(resolve => setTimeout(resolve, 1000));
    return res.json({ success: true, message: 'Booking confirmed (Demo Mode)' });
  }
...
app.get('/api/admin/stats', requireAuth, (req, res) => {
  try {
    const contactsCount = db.prepare("SELECT COUNT(*) as count FROM contacts WHERE status = 'new'").get();
    const bookingsCount = db.prepare("SELECT COUNT(*) as count FROM bookings WHERE date >= date('now')").get();
    const subscribersCount = db.prepare("SELECT COUNT(*) as count FROM subscribers").get();
